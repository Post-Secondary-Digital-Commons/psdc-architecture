...
> Domain: ai
> Owner: PSDC Ai Working Group; accountable maintainer RedjiJB until delegation
> Last reviewed: 2026-09-11

## Purpose and outcome

This specification defines **Agent Runtime** as part of the Post Secondary Digital
...
- **In scope:** behaviour, interfaces, dependencies, data, security, deployment
  ...
  requirements for Agent Runtime.
...
- The Agent Runtime capability SHALL provide provider-neutral AI access, policy enforcement, model routing, tool safety, evaluation, and accountable agent execution.
...
- Governed information includes prompts, responses, embeddings, model metadata, policy decisions, usage records, and explicitly scoped user memory.
...
- Domain controls SHALL include gateway-only model access, prompt-injection defenses, tool allowlists, confirmation levels, data-classification routing, and auditability.
...
- Required lifecycle behaviour includes bounded context, cancellation, rate and token quotas, model fallback, circuit breaking, evaluation gates, and safe degradation.
...
- Adopted boundary and strategy: OpenAPI, OpenAI-compatible semantics, OCI model artifacts where suitable, OpenTelemetry, and replaceable inference runtimes.
...
- Conform to ADR-0004: treat the named engine as an adapter or reference, not as the Commons Compute Fabric contract itself.
...
- ADR-0004: Commons Compute Fabric Is Runtime-Agnostic
