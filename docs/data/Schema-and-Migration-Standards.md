...
This specification defines **Schema and Migration Standards** as part of the Post Secondary Digital
Commons. Its required outcome is managed relational, cache, search, vector, lake, catalog, migration, backup, and recovery capabilities. An implementation conforms
...
- **In scope:** behaviour, interfaces, dependencies, data, security, deployment
  ...
  requirements for Schema and Migration Standards.
...
- The Schema and Migration Standards capability SHALL provide managed relational, cache, search, vector, lake, catalog, migration, backup, and recovery capabilities.
...
- Required interoperability boundary: service-owned data APIs, schema and migration contracts, backup and restore operations, and portable export formats.
...
- Governed information includes classified application records, schemas, indexes, backups, change history, lineage, and retention metadata.
...
- Domain controls SHALL include encryption, row or tenant isolation, credential rotation, audited administrative access, purpose limitation, and verified deletion.
...
- Required lifecycle behaviour includes schema migration, point-in-time recovery, consistency monitoring, compaction, capacity thresholds, and tested restore procedures.
...
- Adopted boundary and strategy: PostgreSQL, Valkey, S3-compatible storage, open search and vector interfaces, and non-proprietary export formats.
...
- Services own their data through documented APIs and migrations; no institution-branded proprietary database primitive is created.
